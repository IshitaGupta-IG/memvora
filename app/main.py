...
from app.services.embeddings import warm_embedding_model
from app.services.memories import build_context, create_memory, delete_memory, get_memory, list_memories, search_memories, update_memory
...
async def warm_models() -> None:
    await run_in_threadpool(warm_embedding_model)
...
async def get_memories(days: int | None = None, user: dict = Depends(get_current_user)) -> dict:
    ...
    memories = await run_in_threadpool(list_memories, user["id"], days=days)
    return {"memories": memories}


@app.get("/memories/{memory_id}")
async def get_memory_detail(memory_id: str, user: dict = Depends(get_current_user)) -> dict:
    memory = await run_in_threadpool(get_memory, user["id"], memory_id)
...
async def edit_memory(memory_id: str, request: MemoryUpdateRequest, user: dict = Depends(get_current_user)) -> UploadResponse:
    ...
    result = await run_in_threadpool(update_memory, user["id"], memory_id, request.title.strip(), request.original_content.strip())
    return UploadResponse(
        memory_id=result["memory"]["id"],
        title=result["memory"]["title"],
        chunks_created=result["chunks_created"],
...
async def remove_memory(memory_id: str, user: dict = Depends(get_current_user)) -> dict:
    await run_in_threadpool(delete_memory, user["id"], memory_id)
...
async def search(query: str, user: dict = Depends(get_current_user)) -> SearchResponse:
    ...
    results = await run_in_threadpool(search_memories, user["id"], query)
...
async def chat(request: ChatRequest, user: dict = Depends(get_current_user)) -> ChatResponse:
    ...
    chunks = await run_in_threadpool(search_memories, user["id"], request.message, limit=6)
    if not chunks:
        return ChatResponse(
            answer="I could not find any saved memories yet. Add a note, link, file, or screenshot, then ask me again.",
            sources=[],
        )
...
async def summary(request: SummaryRequest, user: dict = Depends(get_current_user)) -> SummaryResponse:
    ...
    memories = await run_in_threadpool(list_memories, user["id"], days=request.days, limit=30, include_content=True)
